...
yerr = conf_intervals.confidence_intervals(
    p_coll_results, 'p_coll_dcf_numpy', 'N')

# grouping the results by the same value of N and calculating the mean of
# probability of collision for each N
...
yerr = conf_intervals.confidence_intervals(
    throughput_results, 'throughput_simulation', 'N') / 10e6

# grouping the results with the same N and calculating mean throughput for
# each N
...
throughput_results = throughput_results.merge(
    yerr, on='N', suffixes=('', '_conf'))

# converting throughput into Mbits/s
throughput_results['throughput_simulation'] = throughput_results['throughput_simulation'].div(
    1e6).round(4)
#throughput_results['throughput_simulation_conf'] = throughput_results['throughput_simulation_conf'].div(1e6).round(4)

# merging averege results of throughput and p_coll and saving as
# simulation results
dcf_simulation_results = pd.merge(
    p_coll_results, throughput_results, on=[
        'N', 'cw_min', 'cw_max'])
dcf_simulation_results.to_csv(
